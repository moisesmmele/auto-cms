  ...
  methods: {
    save() {
      ...
      const request = {
        method: method,
        headers: headers,
        body: JSON.stringify({ label: this.localLabel })
      }
      fetch(url, request)
          ...
          .then(({ ok, data }) => {
            if (!ok) {
              notie.alert({
                type: "error",
                text: `Não foi possível atualizar ${this.title}!`
              })
            } else {
              notie.alert({
                type: "success",
                text: `${this.title} criado(a)/atualizado(a) com sucesso!`
              })
              this.toggleModal()
            }
          })
          ...
          <div class="flex flex-wrap gap-x-6 items-center">
            ...
            <div class="flex items-center flex-grow">
              ...
              <input
                  type="text"
                  v-model="localLabel"
                  class="flex-grow border border-gray-300 rounded px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
          </div>
        </div>
